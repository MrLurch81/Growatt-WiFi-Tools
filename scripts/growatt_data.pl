...
# These should be replaced by module functions
...
my $upload = 0;			# Upload to PVOutput
...
sub process_file {
    ...
    # Disassemble.
	my $a = disassemble_datafile( $gwversion, $data );
    unless ( $a ) {
		warn("$file\n");
		return;
    }
...
sub app_options {
    ...
    if ( !GetOptions(
		     'version=i' => \$gwversion,
		     'print!'	=> sub { $print = $_[1]; $export = 0; $upload = 0 },
		     'csv!'		=> sub { $export = $_[1]; $print = 0; $upload = 0 },
		     'upload!'	=> sub { $upload = $_[1]; $print = 0; $export = 0 },
		     'day'		=> \$day,
		     'ident'	=> \$ident,
		     'verbose'	=> \$verbose,
		     'trace'	=> \$trace,
...
Usage: $0 [options] [file ...]
    ...
    --[no]upload		upload data to PVOutput
